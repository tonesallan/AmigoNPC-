...
/**
 * Frases aleatórias para up/down e "épicas" (marcos/ trocas de arma).
 *
 * Usa {lvl} como placeholder.
 */
public final class SwordMessages {
    ...
    public static String pickUp(int lvl) {
        return format(pick(UP), lvl);
    }

    public static String pickDown(int lvl) {
        return format(pick(DOWN), lvl);
    }

    public static String pickEpic(int lvl) {
        return format(pick(EPIC), lvl);
    }
    ...
    private static String format(String s, int lvl) {
        if (s == null) return "";
        return s.replace("{lvl}", String.valueOf(lvl));
    }
}
